...
interface BlitzsiegerEntry {
  year: string;
  winner: string;
}

const BlitzsiegerComponent = () => {
  const [data, setData] = useState<BlitzsiegerEntry[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const response = await fetch('https://sc-laufenburg.de/api/tournaments.php?type=blitzsieger');
        ...
        console.error('Error fetching blitzsieger:', err);
